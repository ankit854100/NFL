...
function InfinteScrollMy(props) {
    ...
    useEffect(() => {
        ...
        let temp = [];
        if(props.excludedPlayers.length >= 15){
            for(let i = 0; i < 15; i++){
                temp.push(props.excludedPlayers[i])
            }
        }
        ...
        // console.log(props.newMy);
    },[props.excludedPlayers])

    const getMoreData = () => {
        // console.log("called");
        setTimeout(() => {
          for(let i = count.prev; i < count.next; i++ ){
            if(i > props.excludedPlayers.length - 1) break;
            // console.log("called",count);
            setMyArray((prev) => {
                return [...prev, props.excludedPlayers[i]];
            })
          }
    ...
    return (
        <div>
            <InfiniteScroll
                ...
                next={getMoreData}
                hasMore={props.position === "ALL" ? myArray.length !== props.excludedPlayers.length: false}
